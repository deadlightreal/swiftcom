...
void frames::ChatRoomFrame::HandleLoadServerInfoResponse(SwiftNetClientPacketData* packet_data) {
...
static void PacketCallback(SwiftNetClientPacketData* restrict const packet_data) {
    ...
    switch(request_info->request_type) {
        ...
        {
            frames::ChatRoomFrame* chat_room_frame = wxGetApp().GetChatRoomFrameById(packet_data->metadata.port_info.source_port);
...
std::vector<frames::ChatRoomFrame::ChatChannel*>* frames::ChatRoomFrame::GetChatChannels() {
    return &this->chat_channels;
}

void frames::ChatRoomFrame::LoadServerInformation() {
    ...
    swiftnet_client_append_to_packet(connection, &request_info, sizeof(request_info), &buffer);

    swiftnet_client_send_packet(connection, &buffer);
}

void frames::ChatRoomFrame::DrawChannels() {
    ...
    for (auto channel : *this->GetChatChannels()) {
        widgets::Button* channel_button = new widgets::Button(this->channel_list_panel, channel->GetName(), [](wxMouseEvent& event){});
...
frames::ChatRoomFrame::ChatRoomFrame(const in_addr ip_address, const uint16_t server_id) : wxFrame(wxGetApp().GetHomeFrame(), wxID_ANY, "Chat Room", wxDefaultPosition, wxSize(800, 600)), server_id(server_id)  {
...
frames::ChatRoomFrame::~ChatRoomFrame() {

}

uint16_t frames::ChatRoomFrame::GetServerId() {
    return this->server_id;
}


SwiftNetClientConnection* frames::ChatRoomFrame::GetConnection() {
